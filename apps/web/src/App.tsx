...
function Shell({ user, logout }: { user: User; logout: () => void }) {
  ...
  return <div className="min-h-screen max-w-full overflow-x-hidden lg:flex">
    ...
    <main className="min-w-0 max-w-full flex-1 overflow-x-hidden pb-20 lg:pb-0">
      <header className="sticky top-0 z-30 flex h-16 max-w-full items-center justify-between gap-3 overflow-hidden border-b bg-cream/90 px-4 backdrop-blur lg:px-8"><button onClick={() => setOpen(true)} className="shrink-0 lg:hidden"><Menu /></button><button onClick={toggleSidebar} title={sidebarHidden?'Tampilkan menu':'Sembunyikan menu'} className="hidden shrink-0 rounded-xl border bg-white p-2 text-slate-600 hover:bg-slate-50 lg:block"><Menu size={20}/></button><div className="min-w-0"><h1 className="truncate font-extrabold">{allowed.find(x => x[0] === loc.pathname)?.[1] || 'FORU POS'}</h1><p className="hidden text-xs text-slate-500 sm:block">Kelola operasional FORU dalam satu tempat</p></div><div className="ml-auto flex min-w-0 shrink-0 items-center gap-2"><button onClick={()=>navigate('/select-outlet')} className="hidden rounded-full bg-white px-3 py-1 text-xs font-black text-slate-600 ring-1 ring-black/5 sm:inline-flex">Outlet Aktif: {activeOutletId ? 'Terpilih' : 'Pilih outlet'}</button><span className="pill bg-brand-100 text-brand-700">{user.role}</span></div></header>
      <Routes>
