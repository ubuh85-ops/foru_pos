...
import { BarChart3, ClipboardList, History, Layers3, LogOut, Menu, Percent, Printer, ReceiptText, RefreshCw, ShoppingBag, Store, Tag, X } from 'lucide-react';
...
import { clearLocalForLogout } from './localDb';
...
export default function App() {
  ...
  return <Routes><Route path="*" element={<Shell user={user} logout={() => { recordLocalAudit('LOGOUT','USER',user.id,{name:user.name}); void clearLocalForLogout(); setUser(null); }} />} /></Routes>;
}
...
function Shell({ user, logout }: { user: User; logout: () => void }) {
  ...
  const allowed = nav.filter(([p]) => user.role === 'OWNER' || !['/coupons', '/outlets', '/categories', '/variant-groups', '/printers'].includes(p));
  ...
  return <div className="min-h-screen max-w-full overflow-x-hidden lg:flex">
    ...
    <main className="min-w-0 max-w-full flex-1 overflow-x-hidden pb-20 lg:pb-0">
      <header className="sticky top-0 z-30 flex h-16 max-w-full items-center justify-between gap-3 overflow-hidden border-b bg-cream/90 px-4 backdrop-blur lg:px-8"><button onClick={() => setOpen(true)} className="shrink-0 lg:hidden"><Menu /></button><button onClick={toggleSidebar} title={sidebarHidden?'Tampilkan menu':'Sembunyikan menu'} className="hidden shrink-0 rounded-xl border bg-white p-2 text-slate-600 hover:bg-slate-50 lg:block"><Menu size={20}/></button><div className="min-w-0"><h1 className="truncate font-extrabold">{allowed.find(x => x[0] === loc.pathname)?.[1] || 'FORU POS'}</h1><p className="hidden text-xs text-slate-500 sm:block">Kelola operasional FORU dalam satu tempat</p></div><div className="ml-auto flex min-w-0 shrink-0 items-center gap-2"><SyncStatus/><span className="pill bg-brand-100 text-brand-700">{user.role}</span></div></header>
      <Routes>
