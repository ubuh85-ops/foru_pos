...
function canOpenInventoryPath(user: User, path: string) {
  ...
  return hasInv(user, tabPermissions[path] || 'inventory.view');
}
...
export default function InventoryPage({ user }: { user: User }) {
  ...
  const warehouseItems = useMemo(() => {
    if (!warehouseId) return activeItems;
    return activeItems.filter(i => stockRows(i, warehouseId).length > 0);
  ...
  async function movement(endpoint: string, payload: any, form?: HTMLFormElement, successMessage = 'Data berhasil disimpan.') {
    ...
    catch (e) { const msg = (e as Error).message; setError(msg); toast.error(msg); throw e; }
  }
  ...
  return <div className="p-4 lg:p-8">
    ...
    {mode === 'adjustments' && <Adjustment items={warehouseItems} warehouses={warehouses} warehouseId={warehouseId} onSubmit={movement} scanForItem={scanForItem} />}
    {mode === 'opname' && <Opname items={warehouseItems} warehouses={warehouses} warehouseId={warehouseId} onSubmit={movement} scanForItem={scanForItem} />}
...
function Items(p: any) {
  return <>
    <div className="mb-4 grid gap-3 lg:grid-cols-[1fr_180px_160px_auto]">
      <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input className="input pl-11" placeholder="Cari nama/kode/SKU barcode..." value={p.q} onChange={(e: any) => p.setQ(e.target.value)} /></div>
      <select className="input" value={p.categoryId} onChange={(e: any) => p.setCategoryId(e.target.value)}><option value="">Semua kategori</option>{p.categories.map((c: Category) => <option value={c.id} key={c.id}>{c.name}</option>)}</select>
      <select className="input" value={p.status} onChange={(e: any) => p.setStatus(e.target.value)}><option value="">Semua status</option><option>ACTIVE</option><option>INACTIVE</option></select>
...
function LegacyItemSelect({ items, value, onChange, warehouseId }: { items: Item[]; value?: string; onChange?: (id: string) => void; warehouseId?: string }) {
  ...
  useEffect(() => {
    ...
    if (onChange) onChange(next); else setInternalId(next);
    setQuery('');
...
function ItemSelect({ items, value, onChange, warehouseId }: { items: Item[]; value?: string; onChange?: (id: string) => void; warehouseId?: string }) {
  ...
  const filtered = items.filter(i => {
    ...
    return i.name.toLowerCase().includes(needle) || i.code.toLowerCase().includes(needle) || (i.sku || '').toLowerCase().includes(needle);
  }).slice(0, 20);
  function choose(id: string) {
    if (onChange) onChange(id); else setInternalId(id);
    setQuery('');
    setOpen(false);
  }
  return <div className="relative">
    <input type="hidden" name="itemId" value={selectedId} required />
