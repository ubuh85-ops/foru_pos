...
export default function InventoryPage({ user }: { user: User }) {
  ...
  useEffect(() => { if (canAccessCurrent) reload(); }, [canAccessCurrent]);
  useEffect(() => { loadItems().catch(e => setError(e.message)); }, [q, categoryId, status, warehouseId]);
...
function stockQty(item: Item, warehouseId?: string) { const rows = stockRows(item, warehouseId); return rows.length ? rows.reduce((sum, s) => sum + n(s.currentQty), 0) : n(item.currentStock); }
function stockAverageCost(item: Item, warehouseId?: string) { const rows = stockRows(item, warehouseId); if (!rows.length) return n(item.averageCost); const qty = rows.reduce((sum, s) => sum + n(s.currentQty), 0); const value = rows.reduce((sum, s) => sum + n(s.currentQty) * n(s.averageCost), 0); return qty > 0 ? value / qty : n(rows[0]?.averageCost ?? item.averageCost); }
