...
const Err = ({ v }: { v: string }) =>
  v ? (
    <div className="mb-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">
      {v}
    </div>
  ) : null;
const Head = ({ title, sub, action }: any) => (
...
const Modal = ({ title, close, children }: any) => (
  <div
  ...
  >
    <div className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-3xl bg-white p-6 shadow-2xl">
      <div className="mb-5 flex items-center justify-between">
        ...
        <button data-back-close="true" onClick={close} className="text-2xl">
          ×
        </button>
      </div>
      {children}
...
const productImageSrc = (url?: string | null) =>
  !url ? "" : url.startsWith("/storage/") ? `${API_ORIGIN}${url}` : url;

function Fields({ items, values = {} }: any) {
  return (
    <>
      {items.map(([name, label, type = "text"]: any) => (
      ...
      <div className="grid gap-3 sm:grid-cols-[132px_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          ...
          <p className="mb-3 text-xs text-slate-400">
            Upload dari galeri atau kamera. File akan dikompres otomatis ke
            WEBP.
          </p>
          <div className="flex flex-wrap gap-2">
            ...
            >
              <FileUp size={16} /> {uploading ? "Mengupload..." : "Upload Foto"}
              <input
                className="hidden"
                type="file"
          ...
          {imageUrl && (
            <p className="mt-2 truncate text-xs text-slate-400">{imageUrl}</p>
          )}
        </div>
      </div>
    </div>
...
export default function ProductPage() {
  ...
  const [importEncoding, setImportEncoding] = useState<"text" | "base64">(
    "text"
  );
  ...
  const summary = useMemo(
    () => ({
      ...
      available: data.filter(
        (product) =>
          outletRow(product)?.isAvailable &&
          outletRow(product)?.isActive &&
          outletRow(product)?.status === "ACTIVE"
      ).length,
      soldOut: data.filter(
        (product) => outletId && !outletRow(product)?.isAvailable
      ).length,
      inactive: data.filter(
        (product) =>
          product.status === "INACTIVE" ||
          (outletId && outletRow(product)?.status === "INACTIVE")
      ).length,
    }),
    [data, outletId]
  );
  async function toggleAvailability(product: any) {
    const row = outletRow(product);
    if (!outletId || !row) return;
    try {
      await api("/menu-availability", {
        method: "PATCH",
        body: JSON.stringify({
          outletId,
      ...
      await load();
      toast.success(
        `${product.name} ${row.isAvailable ? "ditandai habis" : "tersedia"}.`
      );
    } catch (e) {
      toast.error((e as Error).message);
    }
  ...
  const exportAll = async () => {
    try {
      await downloadFile("/products/export", "foru-products-export.csv");
    } catch (e) {
      toast.error((e as Error).message);
    }
  };
  const exportSelected = async () => {
    ...
    try {
      await downloadFile(
        `/products/export?ids=${selectedIds.join(",")}`,
        "foru-products-selected.csv"
      );
    } catch (e) {
      toast.error((e as Error).message);
    }
  ...
  const downloadTemplate = async () => {
    try {
      await downloadFile(
        ...
        "foru-product-import-template.csv"
      );
    } catch (e) {
      toast.error((e as Error).message);
    }
  ...
  async function commitImport() {
    if (!importText.trim())
      return toast.error("Upload file CSV, XLS, atau XLSX terlebih dahulu.");
    setImporting(true);
    try {
      const result: any = await api("/products/import", {
        method: "POST",
        body: importPayload(false),
      });
      toast.success(
        `Import selesai. Baru ${result.summary.imported}, update ${result.summary.updated}, gagal ${result.summary.failed}.`
      );
      setImportOpen(false);
      resetImportState();
  ...
  function downloadErrorReport() {
    const rows = [
      ["Row", "SKU", "Product", "Error"],
      ...(importPreview?.preview || [])
        .filter((x: any) => x.status === "ERROR")
        .map((x: any) => [x.row, x.sku, x.product, x.message]),
    ];
    const csv = rows
      .map((r) =>
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "foru-product-import-errors.csv";
    a.click();
    URL.revokeObjectURL(url);
  }
    ...
    try {
      ...
      const outletPricing = activeOutlets.map((o) => ({
        ...
        outletPrice: f.get(`price_${o.id}`)
          ? Number(f.get(`price_${o.id}`))
          : null,
        outletHpp: f.get(`hpp_${o.id}`) ? Number(f.get(`hpp_${o.id}`)) : null,
        status: f.get(`status_${o.id}`) || "ACTIVE",
      }));
      const channelPricing = activeOutlets.flatMap((o) =>
        ["GOFOOD", "GRABFOOD", "SHOPEEFOOD"].map((channel) => ({
          outletId: o.id,
          channel,
          price: f.get(`channel_${channel}_${o.id}`)
            ? Number(f.get(`channel_${channel}_${o.id}`))
            : null,
  ...
  function rowFor(o: any) {
    return (edit?.outlets || []).find((x: any) => x.outletId === o.id);
  }
  function channelPriceFor(o: any, channel: string) {
    return (
      (edit?.channelPrices || []).find(
        (x: any) => x.outletId === o.id && x.channel === channel
      )?.price ?? ""
    );
  }
  ...
  return (
    <Page>
      <div className="mb-6 flex flex-col justify-between gap-3 xl:flex-row xl:items-end">
        <div>
          <h2 className="text-3xl font-black">Master produk</h2>
          <p className="text-slate-500">
            Produk, harga/HPP outlet, kategori, dan attached variant groups.
          </p>
        </div>
        <div className="flex max-w-full gap-2 overflow-x-auto pb-1">
          <button onClick={() => setEdit({})} className="btn-primary">
            <Plus size={18} /> Tambah
          </button>
          <button onClick={() => setImportOpen(true)} className="btn-soft">
            <FileUp size={18} /> Import
          </button>
          <button onClick={exportAll} className="btn-soft">
            <Download size={18} /> Export
          </button>
          <button onClick={exportSelected} className="btn-soft">
            <FileDown size={18} /> Export Selected
          </button>
      ...
      {selectedOutlet && (
        <div className="mb-4 grid grid-cols-2 gap-3 rounded-3xl bg-brand-50 p-4 sm:grid-cols-4">
          <div>
            <p className="text-xs font-bold text-brand-700">
              {selectedOutlet.name}
            </p>
            <b className="text-xl">{summary.total}</b>
            <p className="text-xs text-slate-500">Total produk</p>
          </div>
          <div>
            <b className="text-xl text-emerald-700">{summary.available}</b>
            <p className="text-xs text-slate-500">Tersedia</p>
          </div>
          <div>
            <b className="text-xl text-amber-700">{summary.soldOut}</b>
            <p className="text-xs text-slate-500">Habis</p>
          </div>
          <div>
            <b className="text-xl text-slate-600">{summary.inactive}</b>
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="bg-slate-50 text-slate-500">
              <tr>
                ...
                <th>SKU</th>
                <th>Kategori</th>
                <th>{selectedOutlet ? "Harga Outlet" : "Base Price"}</th>
                <th>{selectedOutlet ? "Availability" : "Base HPP"}</th>
                <th>{selectedOutlet ? "Outlet" : "Variant Groups"}</th>
                <th>Status</th>
                <th></th>
              </tr>
            </thead>
                ...
                return (
                  <tr className="border-t" key={p.id}>
                    ...
                    <td className="font-bold">
                      {p.name}
                      {selectedOutlet && row?.isRecommended && (
                    ...
                    <td>
                      {selectedOutlet ? (
                        <div>
                          <b>{rupiah(row?.outletPrice ?? p.basePrice)}</b>
                          {row?.outletPrice == null && (
                            <p className="text-xs text-slate-400">
                              Default {rupiah(p.basePrice)}
                            </p>
                          )}
                        </div>
                      ) : (
          ...
          <form onSubmit={save}>
            <Fields
              ...
              items={[
                ...
                ["basePrice", "Base selling price", "number"],
                ["baseHpp", "Base HPP", "number"],
              ]}
            />
            ...
            <select
              ...
              name="categoryId"
              defaultValue={edit.categoryId || categories[0]?.id}
              required
            >
              {categories.map((c) => (
            ...
            <CheckList
              ...
              rows={groups.map((g) => [g.id, g.name])}
              checked={(edit.variantGroups || []).map(
                (x: any) => x.variantGroupId
              )}
            />
            <RecipeSection
              product={edit}
            ...
            <div className="mt-5">
                      ...
                      return (
                        <tr className="border-t" key={o.id}>
                          ...
                          <td className="text-center">
                            <input
                              name={`available_${o.id}`}
                              type="checkbox"
                              defaultChecked={isNew ? true : !!r?.isAvailable}
                            />
                          </td>
                          <td className="text-center">
                            <input
                              name={`recommended_${o.id}`}
                              type="checkbox"
                          ...
                          <td className="p-2">
                            <input
                              ...
                              type="number"
                              min="0"
                              placeholder={`Base ${edit.basePrice ?? 0}`}
                              defaultValue={r?.outletPrice ?? ""}
                            />
                          </td>
                          <td className="p-2">
                            <input
                              ...
                              type="number"
                              min="0"
                              placeholder={`Base ${edit.baseHpp ?? 0}`}
                              defaultValue={r?.outletHpp ?? ""}
                            />
                          </td>
                          {["GOFOOD", "GRABFOOD", "SHOPEEFOOD"].map((ch) => (
                            <td className="p-2" key={ch}>
                              <input
                                className="input"
                                name={`channel_${ch}_${o.id}`}
                                type="number"
                                min="0"
                                placeholder="Fallback Dine In"
                                defaultValue={channelPriceFor(o, ch)}
                              />
                            </td>
                          ))}
                          <td className="p-2">
                            <select
                              ...
                              name={`status_${o.id}`}
                              defaultValue={
                                r?.status || (isNew ? "ACTIVE" : "INACTIVE")
                              }
                            >
                              <option>ACTIVE</option>
                              <option>INACTIVE</option>
              ...
              <p className="mt-2 text-xs text-slate-400">
                Kosongkan harga online untuk memakai harga Dine In outlet. HPP
                tetap memakai HPP produk/outlet.
              </p>
            </div>
      ...
      {importOpen && (
        <Modal title="Import Produk" close={() => setImportOpen(false)}>
          <div className="space-y-4">
            <div className="rounded-2xl bg-brand-50 p-4 text-sm text-brand-800">
              Gunakan template yang tersedia. Import mendukung CSV, XLS, dan
              XLSX. SKU boleh kosong, nanti dibuat otomatis dari nama produk.
              Category kosong akan masuk ke kategori Lainnya; category baru akan
              dibuat otomatis.
            </div>
            <div className="grid gap-3 md:grid-cols-[1fr_220px]">
              <label className="block rounded-2xl border border-dashed border-slate-300 p-5 text-center">
                ...
                <input
                  ...
                  onChange={(e) =>
                    e.target.files?.[0] && readImportFile(e.target.files[0])
                  }
                />
              ...
              <label>
                <span className="label">Mode Import</span>
                <select
                  className="input"
                  value={importMode}
                ...
                >
                  ...
                  <option value="UPDATE_ONLY">Update Existing</option>
                </select>
              </label>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              >
                {importing ? "Memvalidasi..." : "Preview Validasi"}
              </button>
              {importPreview?.summary?.error > 0 && (
                <button className="btn-soft" onClick={downloadErrorReport}>
            ...
            {importPreview && (
              <div className="rounded-2xl border">
                <div className="grid grid-cols-3 gap-2 border-b bg-slate-50 p-3 text-center text-sm">
                  <div>
                    <b>{importPreview.summary.totalRows}</b>
                    <p className="text-slate-400">Rows</p>
                  </div>
                  <div>
                    <b className="text-emerald-600">
                      {importPreview.summary.success}
                    </b>
                    <p className="text-slate-400">Valid</p>
                  </div>
                  <div>
                    <b className="text-red-600">
                      {importPreview.summary.error}
                    </b>
                ...
                <div className="max-h-72 overflow-auto">
                  <table className="w-full min-w-[680px] text-sm">
                    ...
                    <tbody>
                      {importPreview.preview.map((r: any) => (
                        <tr key={r.row} className="border-t">
                          ...
                          <td className="text-red-600">{r.message || "-"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <button
              disabled={
                importing || !importPreview || importPreview.summary.success < 1
              }
            ...
            >
              {importing
                ? "Importing..."
                : `Import ${importPreview?.summary?.success || 0} Produk`}
            </button>
          </div>
        </Modal>
      )}
  ...
  return (
    <section className="mt-5 rounded-3xl border bg-slate-50 p-4">
      ...
      <div className="mt-4 space-y-3">
        ...
        {!rows.length && (
          <div className="rounded-2xl border border-dashed p-4 text-center text-sm text-slate-400">
            Belum ada recipe. Produk tetap bisa dijual tanpa potong stok bahan
            jika outlet mengizinkan.
          </div>
        )}
      </div>
    </section>
    ...
    <div className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-black/5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 gap-2">
          ...
          <div className="min-w-0">
            ...
            {outlet && outletRow?.isRecommended && (
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
        <div className="rounded-2xl bg-slate-50 p-3">
          <p className="text-xs text-slate-400">
            {outlet ? `Harga ${outlet.code}` : "Harga"}
          </p>
          <b>{rupiah(price)}</b>
          {outlet && outletRow?.outletPrice == null && (
            <p className="text-xs text-slate-400">Harga default</p>
          )}
        </div>
        <div className="rounded-2xl bg-slate-50 p-3">
          <p className="text-xs text-slate-400">
            {outlet ? "Availability" : "HPP"}
          </p>
          {outlet ? (
            <button
              onClick={onToggleAvailability}
              className={`font-black ${
                outletRow?.isAvailable ? "text-emerald-700" : "text-amber-700"
              }`}
            >
              {outletRow?.isAvailable ? "● Tersedia" : "○ Habis"}
            </button>
          ) : (
        ...
        <div className="rounded-2xl bg-slate-50 p-3">
          <p className="text-xs text-slate-400">
            {outlet ? "Outlet" : "Outlet aktif"}
          </p>
          <b>
            {outlet?.name ||
              `${
                (p.outlets || []).filter(
                  (x: any) => x.isAvailable && x.status === "ACTIVE"
                ).length
              } outlet`}
          </b>
        </div>
