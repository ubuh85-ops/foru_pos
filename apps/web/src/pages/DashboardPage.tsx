...
export default function DashboardPage() {
  ...
  return <Page>
    <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
      ...
      {user?.role === 'OWNER' && <div className="flex rounded-2xl bg-white p-1 shadow-sm ring-1 ring-black/5"><button onClick={() => setMode('outlet')} className={`rounded-xl px-4 py-2 text-sm font-black ${mode === 'outlet' ? 'bg-brand-500 text-white' : 'text-slate-500'}`}>Outlet Aktif</button><button onClick={() => setMode('all')} className={`rounded-xl px-4 py-2 text-sm font-black ${mode === 'all' ? 'bg-brand-500 text-white' : 'text-slate-500'}`}>Semua Outlet</button></div>}
    </div>
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-3"><div className="card p-5 xl:col-span-2"><h3 className="section-title mb-5">Performa outlet</h3>{data?.outlets?.length ? <div className="overflow-auto"><table className="w-full text-left text-sm"><thead><tr><th className="p-3">Outlet</th><th>Net sales</th><th>Transaksi</th><th>Profit</th></tr></thead><tbody>{data.outlets.map((x: any) => <tr className="border-t" key={x.outlet}><td className="py-4 font-bold">{x.outlet}</td><td>{rupiah(x.netSales)}</td><td>{x.transactions}</td><td className="text-brand-700">{rupiah(x.grossProfit)}</td></tr>)}</tbody></table></div> : <div className="p-6 text-center text-slate-400">Belum ada data.</div>}</div><div className="card bg-brand-700 p-5 text-white"><p className="text-sm text-white/70">Gross margin</p><b className="mt-4 block text-5xl">{data?.grossMargin || 0}%</b><p className="mt-5 text-sm text-white/70">Net sales setelah seluruh diskon dikurangi HPP.</p></div></div>
  </Page>;
}
