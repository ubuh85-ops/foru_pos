...
function publicProductShape(p:any,outletId:string,channel='DINE_IN',stock=productStockSnapshot(p.outlets?.[0])){
...
api.get('/public/order/:businessSlug/:outletSlug/products',asyncRoute(async(req,res)=>{
  ...
  res.json(await Promise.all(products.map(async p=>publicProductShape(p,outlet.id,'DINE_IN',await effectiveProductStock(p.id,p.outlets[0],outlet)))));
}));
...
const pushDeviceBody=z.object({
  ...
  isPosActive:z.coerce.boolean().default(false),
  soundEnabled:z.coerce.boolean().default(true),
  soundName:z.string().trim().max(40).optional().nullable()
});
api.post('/push-devices',asyncRoute(async(req,res)=>{
  ...
  const device=await prisma.pushDevice.upsert({
    where:{token:d.token},
    create:{token:d.token,platform:d.platform,deviceName:d.deviceName,userId:req.user!.id,businessId:req.user!.businessId,outletId:d.outletId,isPosActive:d.isPosActive,posLastSeenAt:d.isPosActive?new Date():null,soundEnabled:d.soundEnabled,soundName:d.soundName||null},
    update:{platform:d.platform,deviceName:d.deviceName,userId:req.user!.id,businessId:req.user!.businessId,outletId:d.outletId,isActive:true,isPosActive:d.isPosActive,posLastSeenAt:d.isPosActive?new Date():null,soundEnabled:d.soundEnabled,soundName:d.soundName||null,lastSeenAt:new Date()}
  });
...
const pushPresenceBody=z.object({outletId:z.string().trim().min(1),isPosActive:z.coerce.boolean()});
api.put('/push-devices/current/presence',asyncRoute(async(req,res)=>{
  const d=pushPresenceBody.parse(req.body);
  await assertTenantOutlet(req,d.outletId);
  const now=new Date();
...
type OnlineChannel=typeof onlineChannels[number];
type OnlineFeeRates=Record<OnlineChannel,number>;
const onlineFeeSettingsSelect={gofoodFeePercent:true,grabfoodFeePercent:true,shopeefoodFeePercent:true} as const;
function onlineFeeRates(b:any):OnlineFeeRates{return {GOFOOD:Number(b?.gofoodFeePercent||0),GRABFOOD:Number(b?.grabfoodFeePercent||0),SHOPEEFOOD:Number(b?.shopeefoodFeePercent||0)};}
function saleOnlineChannel(s:any):OnlineChannel|null{const orderType=String(s?.orderType||'').toUpperCase();if(onlineChannels.includes(orderType as OnlineChannel))return orderType as OnlineChannel;const payment=String(s?.paymentMethod||'').toUpperCase();return onlineChannels.includes(payment as OnlineChannel)?payment as OnlineChannel:null;}
function onlineFeeFor(amount:number,sale:any,rates:OnlineFeeRates){const channel=saleOnlineChannel(sale);return channel?money(amount*(rates[channel]||0)/100):0;}
const onlineFeeBody=z.object({gofoodFeePercent:z.coerce.number().min(0).max(100),grabfoodFeePercent:z.coerce.number().min(0).max(100),shopeefoodFeePercent:z.coerce.number().min(0).max(100)});
api.get('/settings/online-fees',allow('OWNER'),asyncRoute(async(req,res)=>{const business=await prisma.business.findUnique({where:{id:req.user!.businessId},select:onlineFeeSettingsSelect});if(!business)throw new ApiError(403,'Business tidak valid');res.json(onlineFeeBody.parse(business));}));
api.put('/settings/online-fees',allow('OWNER'),asyncRoute(async(req,res)=>{const data=onlineFeeBody.parse(req.body);const business=await prisma.business.update({where:{id:req.user!.businessId},data,select:onlineFeeSettingsSelect});res.json(onlineFeeBody.parse(business));}));
...
const productStockSettingsBody=z.object({outletId:z.string(),productId:z.string(),stockMode:z.enum(['UNLIMITED','MANUAL','RECIPE']),lowStockThreshold:z.coerce.number().int().nonnegative().max(1000000).default(5)});
api.patch('/menu-availability/stock-settings',allow('OWNER','SUPERVISOR'),asyncRoute(async(req,res)=>{
  const d=productStockSettingsBody.parse(req.body);
  await assertTenantOutlet(req,d.outletId);await assertTenantProduct(req,d.productId);
  const existing=await prisma.productOutlet.findFirst({where:{outletId:d.outletId,productId:d.productId,status:'ACTIVE',outlet:{businessId:req.user!.businessId},product:{businessId:req.user!.businessId}}});
  if(!existing)throw new ApiError(404,'Produk tidak terdaftar aktif di outlet ini');
  const updated=await prisma.$transaction(async tx=>{
    const row=await tx.productOutlet.update({where:{productId_outletId:{productId:d.productId,outletId:d.outletId}},data:{stockMode:d.stockMode,lowStockThreshold:d.lowStockThreshold,stockUpdatedAt:new Date(),stockUpdatedBy:req.user!.id}});
    await tx.auditLog.create({data:{businessId:req.user!.businessId,entityType:'PRODUCT_OUTLET',entityId:`${d.productId}:${d.outletId}`,action:'PRODUCT_STOCK_SETTINGS_UPDATED',oldValue:{stockMode:existing.stockMode,lowStockThreshold:existing.lowStockThreshold},newValue:{stockMode:d.stockMode,lowStockThreshold:d.lowStockThreshold},changedBy:req.user!.id}});
    return row;
  });
  res.json(updated);
}));
const productStockUpdateBody=z.object({outletId:z.string(),productId:z.string(),stockQty:z.coerce.number().int().nonnegative().max(1000000),reason:z.string().trim().min(3).max(250)});
api.patch('/menu-availability/stock',allow('OWNER','SUPERVISOR','CASHIER'),asyncRoute(async(req,res)=>{
  const d=productStockUpdateBody.parse(req.body);
  await assertTenantOutlet(req,d.outletId);await assertTenantProduct(req,d.productId);
  const existing=await prisma.productOutlet.findFirst({where:{outletId:d.outletId,productId:d.productId,status:'ACTIVE',outlet:{businessId:req.user!.businessId},product:{businessId:req.user!.businessId}}});
  if(!existing)throw new ApiError(404,'Produk tidak terdaftar aktif di outlet ini');
  if(existing.stockMode!=='MANUAL')throw new ApiError(400,'Update jumlah hanya tersedia untuk mode stok manual');
  const updated=await prisma.$transaction(async tx=>{
    const row=await tx.productOutlet.update({where:{productId_outletId:{productId:d.productId,outletId:d.outletId}},data:{stockQty:d.stockQty,stockUpdatedAt:new Date(),stockUpdatedBy:req.user!.id}});
    await tx.productStockMovement.create({data:{businessId:req.user!.businessId,outletId:d.outletId,productId:d.productId,movementType:'MANUAL_SET',qty:d.stockQty-existing.stockQty,beforeQty:existing.stockQty,afterQty:d.stockQty,reason:d.reason,createdBy:req.user!.id}});
    await tx.auditLog.create({data:{businessId:req.user!.businessId,entityType:'PRODUCT_STOCK',entityId:`${d.productId}:${d.outletId}`,action:'PRODUCT_STOCK_SET',oldValue:{stockQty:existing.stockQty},newValue:{stockQty:d.stockQty,reason:d.reason},changedBy:req.user!.id}});
    return row;
  });
  res.json({...updated,...productStockSnapshot(updated)});
}));
...
async function buildShiftCloseReport(id:string){
  ...
  const feeBusiness=await prisma.business.findUnique({where:{id:s.businessId},select:onlineFeeSettingsSelect});
  const feeRates=onlineFeeRates(feeBusiness);
  const onlineFee=paid.reduce((n:number,x:any)=>n+onlineFeeFor(Number(x.grandTotal||0),x,feeRates),0);
  ...
  return {id:s.id,shiftId:s.id,shiftNumber:shiftNumberOf(s),status:s.status,outlet:s.outlet,openedBy:s.cashier,closedBy:s.closedBy,openedAt:s.openedAt,closedAt:s.closedAt,openingCash:money(Number(s.openingCash)),cashSummary:{openingCash:money(Number(s.openingCash)),cashSales:money(cashSales),cashDrawerExpenses:money(cashDrawerExpenses),cashRefund,expectedCash,actualCash,variance},omsetSummary:{grossSales:money(grossSales),productDiscount:money(productDiscount),transactionDiscount:money(transactionDiscount),couponDiscount:money(couponDiscount),discount:money(productDiscount+transactionDiscount+couponDiscount),netSales:money(netSales),totalOmset:money(netSales),totalHpp:money(totalHpp),onlineFee:money(onlineFee),grossProfit:money(netSales-totalHpp-onlineFee)},paymentBreakdown:paymentBreakdown(paid),expenseSummary:expenseSummary(s.expenses),itemSold:[...itemMap.values()].sort((a,b)=>b.qty-a.qty),orderSummary,expenseDetails:s.expenses,printerLogs:s.printerLogs,expectedCash,closingCashActual:actualCash,variance,cashSales:money(cashSales),cashDrawerExpenses:money(cashDrawerExpenses),cashRefund};
}
...
async function deductManualProductStock(tx:any,sale:any,userId:string,stockRows:any[]){
  const byProduct=new Map(stockRows.map(row=>[row.productId,row]));
  const quantities=new Map<string,{qty:number;name:string}>();
  for(const item of sale.items||[]){
    const current=quantities.get(item.productId)||{qty:0,name:item.productName};
    current.qty+=Number(item.qty);quantities.set(item.productId,current);
  }
  for(const [productId,item] of quantities){
    const stock=byProduct.get(productId);
    if(!stock||stock.stockMode!=='MANUAL')continue;
    const updated=await tx.$queryRaw<Array<{stock_qty:number}>>`
      UPDATE "product_outlets"
      SET "stock_qty" = "stock_qty" - ${item.qty}, "stock_updated_at" = NOW(), "stock_updated_by" = ${userId}, "updated_at" = NOW()
      WHERE "product_id" = ${productId} AND "outlet_id" = ${sale.outletId} AND "stock_mode" = 'MANUAL' AND "stock_qty" >= ${item.qty}
      RETURNING "stock_qty"
    `;
    if(!updated.length)throw new ApiError(409,`Stok ${item.name} tidak mencukupi. Silakan perbarui pesanan.`);
    const afterQty=Number(updated[0].stock_qty),beforeQty=afterQty+item.qty;
    await tx.productStockMovement.create({data:{businessId:sale.businessId,outletId:sale.outletId,productId,saleId:sale.id,movementType:'SALE_DEDUCTION',qty:-item.qty,beforeQty,afterQty,reason:`Penjualan ${sale.transactionNumber||sale.orderNumber}`,createdBy:userId}});
  }
}
async function deductInventoryForPaidSale(tx:any,sale:any,userId:string){
  ...
  const stockByProduct=new Map(productStocks.map((row:any)=>[row.productId,row]));
...
async function returnInventoryForVoidedSale(tx:any,sale:any,userId:string){
  ...
  for(const movement of productMovements){
    ...
    const row=await tx.productOutlet.findUnique({where:{productId_outletId:{productId:movement.productId,outletId:movement.outletId}}});
    if(!row)continue;
    const beforeQty=Number(row.stockQty),afterQty=beforeQty+qty;
    await tx.productOutlet.update({where:{id:row.id},data:{stockQty:{increment:qty},stockUpdatedAt:new Date(),stockUpdatedBy:userId}});
    await tx.productStockMovement.create({data:{businessId:sale.businessId,outletId:movement.outletId,productId:movement.productId,saleId:sale.id,movementType:'SALE_VOID_RETURN',qty,beforeQty,afterQty,reason:`Void ${sale.transactionNumber||sale.orderNumber}`,createdBy:userId}});
  }
...
async function reportLines(req:any){
  ...
  for(const sale of sales as any[]){
    ...
    for(const item of matching){
      ...
      const net=afterProduct-orderDiscount;
      lines.push(roundReportLine({saleId:sale.id,productId:item.productId,productName:item.productName,categoryId:cat.categoryId,categoryName:cat.categoryName,variantName:item.variantName||'Base',qty:Number(item.qty||0),gross,productDiscount,orderDiscount,discount:productDiscount+orderDiscount,net,cogs,unitHpp:Number(item.finalUnitHpp||item.hpp||0),onlineFee:onlineFeeFor(net,sale,feeRates),paymentMethod:sale.paymentMethod||null}));
    }
  }
  return lines;
}
function groupReport(lines:ReportLine[],key:(line:ReportLine)=>string,base:(line:ReportLine)=>any){
  ...
  for(const line of lines){
    const k=key(line),row=map.get(k)||{...base(line),qtySold:0,grossSales:0,discount:0,netSales:0,cogs:0,onlineFee:0,grossProfit:0,orderIds:new Set<string>(),unitHppTotal:0};
    row.qtySold+=line.qty; row.grossSales+=line.gross; row.discount+=line.discount; row.netSales+=line.net; row.cogs+=line.cogs; row.onlineFee+=line.onlineFee; row.grossProfit+=line.net-line.cogs-line.onlineFee; row.unitHppTotal+=line.unitHpp*line.qty; row.orderIds.add(line.saleId); map.set(k,row);
  }
  return [...map.values()].map(row=>{const orderCount=row.orderIds.size;delete row.orderIds;const grossMarginPercent=row.netSales?money(row.grossProfit/row.netSales*100):0;const unitHpp=row.qtySold?money(row.unitHppTotal/row.qtySold):0;delete row.unitHppTotal;return {...row,orderCount,unitHpp,qtySold:Number(row.qtySold),grossSales:money(row.grossSales),discount:money(row.discount),netSales:money(row.netSales),cogs:money(row.cogs),onlineFee:money(row.onlineFee),grossProfit:money(row.grossProfit),grossMarginPercent};}).sort((a,b)=>b.qtySold-a.qtySold);
}
...
async function report(req:any,consolidated=false){if(consolidated&&req.user.role!=='OWNER')throw new ApiError(403,'Hanya OWNER yang dapat melihat laporan konsolidasi.');const range=req.query.date?dayRange(String(req.query.date)):dayRange();const baseWhere:any=tenantWhereAnd(req,{createdAt:range});if(!consolidated)baseWhere.outletId=await requiredOutletId(req);const [rawSales,expenses,pending,feeBusiness]=await Promise.all([prisma.sale.findMany({where:{...baseWhere,status:'PAID'},include:{items:true,outlet:true}}),prisma.expense.findMany({where:{...baseWhere,status:'ACTIVE'},include:{categoryRef:true,outlet:true}}),prisma.sale.aggregate({where:{...baseWhere,status:'PENDING_PAYMENT'},_count:true,_sum:{grandTotal:true}}),prisma.business.findUnique({where:{id:req.user!.businessId},select:onlineFeeSettingsSelect})]);const feeRates=onlineFeeRates(feeBusiness);const sales=rawSales.map(s=>{const onlineFee=onlineFeeFor(Number(s.grandTotal),s,feeRates);return Object.assign(s,{reportOnlineFee:onlineFee,reportGrossProfit:money(Number(s.grandTotal)-Number(s.totalHpp)-onlineFee)});});const sum=(key:string)=>money(sales.reduce((n,s)=>n+Number((s as any)[key]),0));const gross=sum('subtotalBeforeDiscount'),productDiscount=sum('productDiscountTotal'),transactionDiscount=sum('transactionDiscountAmount'),couponDiscount=sum('couponDiscountAmount'),net=sum('grandTotal'),hpp=sum('totalHpp'),onlineFee=sum('reportOnlineFee');const cashSales=money(sales.filter(s=>s.paymentMethod==='CASH').reduce((n,s)=>n+Number(s.grandTotal),0));const cashDrawerExpense=money(expenses.filter(e=>e.paymentSource==='CASH_DRAWER').reduce((n,e)=>n+Number(e.amount),0));const nonCashExpense=money(expenses.filter(e=>e.paymentSource==='NON_CASH').reduce((n,e)=>n+Number(e.amount),0));const ownerTransferExpense=money(expenses.filter(e=>e.paymentSource==='OWNER_TRANSFER').reduce((n,e)=>n+Number(e.amount),0));const expenseByCategory=[...expenses.reduce((m,e)=>{const k=e.categoryName||e.categoryRef?.name||'Lain-lain';m.set(k,(m.get(k)||0)+Number(e.amount));return m;},new Map<string,number>()).entries()].map(([category,amount])=>({category,amount:money(amount)}));const grossProfit=money(net-hpp-onlineFee);return {grossSales:gross,productDiscount,transactionDiscount,couponDiscount,netSales:net,paidSalesAmount:net,pendingOrdersCount:pending._count,pendingOrdersAmount:money(Number(pending._sum.grandTotal||0)),totalHpp:hpp,onlineFee,grossProfit,grossMargin:net?money(grossProfit/net*100):0,totalTransactions:sales.length,averageTicket:sales.length?money(net/sales.length):0,payments:Object.fromEntries(['CASH','QRIS','GOFOOD','GRABFOOD','SHOPEEFOOD','VOUCHER'].map(p=>[p,money(sales.filter(s=>s.paymentMethod===p).reduce((n,s)=>n+Number(s.grandTotal),0))])),cashDrawerExpense,nonCashExpense,ownerTransferExpense,totalExpense:money(expenses.reduce((n,e)=>n+Number(e.amount),0)),expenseByCategory,netCashMovement:money(cashSales-cashDrawerExpense),sales};}
