...
async function assertTenantOutlet(req:any,outletId:string){
  ...
  const outlet=await prisma.outlet.findFirst({where:{id:outletId,status:'ACTIVE',OR:[{businessId:req.user!.businessId},{businessId:null}]}});
...
api.post('/users',allow('OWNER'),asyncRoute(async(req,res)=>{
  ...
  const created=await prisma.$transaction(async tx=>{
    ...
    await tx.auditLog.create({data:{entityType:'USER',entityId:user.id,action:'USER_CREATED',oldValue:Prisma.JsonNull,newValue:{name:d.name,username:d.username,role:d.role,status:d.status,outletIds:d.outletIds,inventoryPermissions,assignedWarehouseId},changedBy:req.user!.id}});
...
api.get('/outlets',asyncRoute(async(req,res)=>res.json(await prisma.outlet.findMany({where:req.user!.role==='OWNER'?{OR:[{businessId:req.user!.businessId},{businessId:null}]}:{id:{in:req.user!.outletIds}},include:outletInclude,orderBy:{name:'asc'}}))));
api.post('/outlets',allow('OWNER'),asyncRoute(async(req,res)=>res.status(201).json(await prisma.outlet.create({data:{...z.object({code:z.string().min(2),name:z.string().min(2),address:z.string().optional(),phone:z.string().optional(),inventoryWarehouseId:z.string().nullable().optional(),blockSaleWhenIngredientOutOfStock:z.coerce.boolean().optional(),allowSaleWithoutRecipe:z.coerce.boolean().optional(),...outletPrintSettingsSchema}).parse(req.body),businessId:req.user!.businessId},include:outletInclude}))));
api.put('/outlets/:id',allow('OWNER'),asyncRoute(async(req,res)=>{const id=String(req.params.id);await assertTenantOutlet(req,id);res.json(await prisma.outlet.update({where:{id},data:z.object({code:z.string().min(2).optional(),name:z.string().min(2).optional(),address:z.string().nullable().optional(),phone:z.string().nullable().optional(),inventoryWarehouseId:z.string().nullable().optional(),blockSaleWhenIngredientOutOfStock:z.coerce.boolean().optional(),allowSaleWithoutRecipe:z.coerce.boolean().optional(),...outletPrintSettingsSchema,status:z.enum(['ACTIVE','INACTIVE']).optional()}).parse(req.body),include:outletInclude}));}));
...
api.get('/printers',allow('OWNER','SUPERVISOR','CASHIER'),asyncRoute(async(req,res)=>{const where:any={OR:[{businessId:req.user!.businessId},{businessId:null}]};if(req.query.outlet_id){assertOutlet(req,String(req.query.outlet_id));where.outletId=String(req.query.outlet_id);}else if(req.user!.role!=='OWNER')where.outletId={in:req.user!.outletIds};res.json(await prisma.printer.findMany({where,include:{outlet:true},orderBy:{createdAt:'desc'}}));}));
api.post('/printers',allow('OWNER'),asyncRoute(async(req,res)=>{const d=printerBody.parse(req.body);assertOutlet(req,d.outletId);res.status(201).json(await prisma.printer.create({data:{...d,businessId:req.user!.businessId},include:{outlet:true}}));}));
api.put('/printers/:id',allow('OWNER'),asyncRoute(async(req,res)=>{const d=printerBody.partial().parse(req.body);const existing=await assertTenantPrinter(req,String(req.params.id));assertOutlet(req,d.outletId||existing.outletId);res.json(await prisma.printer.update({where:{id:existing.id},data:d,include:{outlet:true}}));}));
...
api.get('/categories',asyncRoute(async(req,res)=>res.json(await prisma.category.findMany({where:{OR:[{businessId:req.user!.businessId},{businessId:null}]},orderBy:[{sortOrder:'asc'},{name:'asc'}]}))));
...
api.get('/variant-groups',asyncRoute(async(req,res)=>res.json(await prisma.variantGroup.findMany({where:{OR:[{businessId:req.user!.businessId},{businessId:null}]},include:{options:{orderBy:{sortOrder:'asc'}}},orderBy:{name:'asc'}}))));
...
api.get('/products',asyncRoute(async(req,res)=>res.json(await prisma.product.findMany({where:{OR:[{businessId:req.user!.businessId},{businessId:null}]},include:productInclude,orderBy:{name:'asc'}}))));
...
api.post('/products/import',allow('OWNER','SUPERVISOR'),asyncRoute(async(req,res)=>{const d=z.object({filename:z.string().optional(),mode:z.enum(['INSERT_ONLY','UPDATE_ONLY','UPSERT']).default('UPSERT'),preview:z.coerce.boolean().default(true),content:z.string().min(1),encoding:z.enum(['text','base64']).default('text')}).parse(req.body);const validated=await validateProductImportRows(req,d.content,d.mode,d.filename,d.encoding);if(d.preview)return res.json(validated);let imported=0,updated=0,failed=0;const results=[] as any[];for(const p of validated.products){const rowErrors=validated.preview.filter(x=>x.sku===p.sku&&x.status==='ERROR');if(rowErrors.length){failed++;results.push({sku:p.sku,status:'ERROR',message:rowErrors.map(x=>x.message).join('; ')});continue;}try{await prisma.$transaction(async tx=>{const existing=p.sku?await tx.product.findFirst({where:tenantWhereAnd(req,{sku:p.sku})}):null;if(existing){if(d.mode==='INSERT_ONLY')throw new Error('Duplicate SKU database');if(p.variantGroupIds){await tx.productVariantGroup.deleteMany({where:{productId:existing.id}});await tx.productVariantGroup.createMany({data:p.variantGroupIds.map((variantGroupId:string,i:number)=>({productId:existing.id,variantGroupId,sortOrder:i})),skipDuplicates:true});}for(const x of p.outletPricing)await tx.productOutlet.upsert({where:{productId_outletId:{productId:existing.id,outletId:x.outletId}},update:{isAvailable:x.isAvailable,isActive:x.isAvailable,status:x.status,outletPrice:x.outletPrice,outletHpp:x.outletHpp},create:{productId:existing.id,outletId:x.outletId,isAvailable:x.isAvailable,isActive:x.isAvailable,status:x.status,outletPrice:x.outletPrice,outletHpp:x.outletHpp}});for(const x of p.channelPricing||[]){if(x.price==null||x.status==='INACTIVE')await tx.productChannelPrice.deleteMany({where:{productId:existing.id,outletId:x.outletId,channel:x.channel}});else await tx.productChannelPrice.upsert({where:{productId_outletId_channel:{productId:existing.id,outletId:x.outletId,channel:x.channel}},update:{price:x.price,status:x.status},create:{productId:existing.id,outletId:x.outletId,channel:x.channel,price:x.price,status:x.status}});}const updatedProduct=await tx.product.update({where:{id:existing.id},data:{businessId:existing.businessId||req.user!.businessId,name:p.name,category:p.category,categoryId:p.categoryId,description:p.description||null,imageUrl:p.imageUrl||null,basePrice:p.basePrice,baseHpp:p.baseHpp,status:p.status}});const baseVariant=await tx.productVariant.findFirst({where:{productId:existing.id,variantName:'Base'}});if(baseVariant)await tx.productVariant.update({where:{id:baseVariant.id},data:{sellingPrice:p.basePrice,hpp:p.baseHpp}});else await tx.productVariant.create({data:{productId:existing.id,variantName:'Base',sellingPrice:p.basePrice,hpp:p.baseHpp}});updated++;return updatedProduct;}else{if(d.mode==='UPDATE_ONLY')throw new Error('SKU tidak ditemukan untuk update');const channelRows=(p.channelPricing||[]).filter((x:any)=>x.price!=null&&x.status==='ACTIVE');await tx.product.create({data:{businessId:req.user!.businessId,sku:p.sku,name:p.name,category:p.category,categoryId:p.categoryId,description:p.description||null,imageUrl:p.imageUrl||null,basePrice:p.basePrice,baseHpp:p.baseHpp,status:p.status,variants:{create:{variantName:'Base',sellingPrice:p.basePrice,hpp:p.baseHpp}},variantGroups:{create:p.variantGroupIds.map((variantGroupId:string,i:number)=>({variantGroupId,sortOrder:i}))},outlets:{create:p.outletPricing.map((x:any)=>({outletId:x.outletId,isAvailable:x.isAvailable,isActive:x.isAvailable,status:x.status,outletPrice:x.outletPrice,outletHpp:x.outletHpp}))},channelPrices:{create:channelRows.map((x:any)=>({outletId:x.outletId,channel:x.channel,price:x.price,status:x.status}))}}});imported++;}});results.push({sku:p.sku,status:'OK'});}catch(e){failed++;results.push({sku:p.sku,status:'ERROR',message:(e as Error).message});}}await prisma.auditLog.create({data:{businessId:req.user!.businessId,entityType:'PRODUCT_IMPORT',entityId:`import-${Date.now()}`,action:'PRODUCT_IMPORT',oldValue:Prisma.JsonNull,newValue:{filename:d.filename,imported,updated,failed,total:validated.products.length},changedBy:req.user!.id}});res.json({summary:{imported,updated,failed,total:validated.products.length},results});}));
api.get('/pos/products',asyncRoute(async(req,res)=>{ const outletId=String(req.query.outlet_id||''); assertOutlet(req,outletId); const channel=String(req.query.channel||req.query.orderType||'DINE_IN').toUpperCase(); const onlineChannel=onlineChannels.includes(channel as any)?channel as typeof onlineChannels[number]:undefined; const products=await prisma.product.findMany({where:{status:'ACTIVE',OR:[{businessId:req.user!.businessId},{businessId:null}],outlets:{some:{outletId,isAvailable:true,isActive:true,status:'ACTIVE'}}},include:{categoryRef:true,outlets:{where:{outletId}},channelPrices:onlineChannel?{where:{outletId,channel:onlineChannel,status:'ACTIVE'}}:false,variants:{where:{status:'ACTIVE'}},addons:{where:{status:'ACTIVE'}},variantGroups:{orderBy:{sortOrder:'asc'},include:{group:{include:{options:{where:{status:'ACTIVE'},orderBy:{sortOrder:'asc'},include:{outlets:{where:{outletId}}}}}}}}},orderBy:{name:'asc'}});res.json(products.map(p=>{const po=p.outlets[0];const dineInPrice=po?.outletPrice??p.basePrice;const cp=(p as any).channelPrices?.[0];const activePrice=cp?.price??dineInPrice;return {...p,basePrice:activePrice,baseHpp:po?.outletHpp??p.baseHpp,masterBasePrice:p.basePrice,masterBaseHpp:p.baseHpp,dineInPrice,channelPrice:cp?.price??null,priceChannel:onlineChannel||'DINE_IN',priceSource:cp?'CHANNEL':(po?.outletPrice!=null?'OUTLET':'BASE'),variantGroups:p.variantGroups.map(vg=>({...vg,group:{...vg.group,options:vg.group.options.filter(o=>!o.outlets[0]||o.outlets[0].status==='ACTIVE').map(o=>({...o,additionalPrice:o.outlets[0]?.additionalPrice??o.additionalPrice,hpp:o.outlets[0]?.hpp??o.hpp,masterAdditionalPrice:o.additionalPrice,masterHpp:o.hpp}))}}))};})); }));
api.post('/products',allow('OWNER','SUPERVISOR'),asyncRoute(async(req,res)=>{
  ...
  if(!outletRows.length){
    const outlets=await prisma.outlet.findMany({where:{status:'ACTIVE',OR:[{businessId:req.user!.businessId},{businessId:null}]},select:{id:true}});
...
api.put('/products/:id',allow('OWNER','SUPERVISOR'),asyncRoute(async(req,res)=>{
  ...
  res.json(await prisma.$transaction(async tx=>{
    ...
    const updated=await tx.product.update({where:{id},data:{businessId:current.businessId||req.user!.businessId,sku:d.sku===undefined?undefined:(d.sku?.trim()||null),name:d.name,category,categoryId:d.categoryId,description:d.description,imageUrl:d.imageUrl||undefined,basePrice:d.basePrice,baseHpp:d.baseHpp,status:d.status},include:productInclude});
...
api.get('/expense-categories',asyncRoute(async(req,res)=>res.json(await prisma.expenseCategory.findMany({where:{OR:[{businessId:req.user!.businessId},{businessId:null}]},orderBy:[{sortOrder:'asc'},{name:'asc'}]}))));
...
async function getDefaultWarehouse(tx:any=prisma,businessId?:string){let wh=await tx.inventoryWarehouse.findFirst({where:{status:'ACTIVE',...(businessId?{OR:[{businessId},{businessId:null}]}:{})},orderBy:{createdAt:'asc'}});if(!wh)wh=await tx.inventoryWarehouse.create({data:{businessId,code:'WH-DEFAULT',name:'Gudang Utama',type:'CENTRAL',status:'ACTIVE'}});return wh;}
...
function warehouseScope(req:any){
  if(req.user!.role==='OWNER')return {OR:[{businessId:req.user!.businessId},{businessId:null}]};
  if(req.user!.assignedWarehouseId)return {id:req.user!.assignedWarehouseId};
  return {outletId:{in:req.user!.outletIds},OR:[{businessId:req.user!.businessId},{businessId:null}]};
}
...
async function warehouseIdForRequest(req:any,id?:string|null,tx:any=prisma){if(id){await assertWarehouseAccess(req,id,tx);return id;}if(req.user!.role==='OWNER')return warehouseIdOrDefault(null,tx,req.user!.businessId);if(req.user!.assignedWarehouseId){await assertWarehouseAccess(req,req.user!.assignedWarehouseId,tx);return req.user!.assignedWarehouseId;}const wh=await tx.inventoryWarehouse.findFirst({where:{status:'ACTIVE',outletId:{in:req.user!.outletIds},OR:[{businessId:req.user!.businessId},{businessId:null}]},orderBy:{name:'asc'}});if(!wh)throw new ApiError(403,'Warehouse tidak diizinkan');return wh.id;}
...
api.post('/warehouses',invWarehouse,asyncRoute(async(req,res)=>{const d=warehouseBody.parse(req.body);if(d.outletId)assertOutlet(req,d.outletId);if(req.user!.role!=='OWNER'&&!d.outletId)throw new ApiError(403,'Warehouse harus terhubung ke outlet yang diassign.');if(await prisma.inventoryWarehouse.findUnique({where:{code:d.code}}))throw new ApiError(409,'Kode warehouse sudah digunakan');res.status(201).json(await prisma.inventoryWarehouse.create({data:{...d,businessId:req.user!.businessId},include:{outlet:true}}));}));
api.put('/warehouses/:id',invWarehouse,asyncRoute(async(req,res)=>{await assertWarehouseAccess(req,String(req.params.id));const d=warehouseBody.partial().parse(req.body);if(d.outletId)assertOutlet(req,d.outletId);res.json(await prisma.inventoryWarehouse.update({where:{id:String(req.params.id)},data:d,include:{outlet:true}}));}));
...
api.get('/inventory/alerts/check',invView,asyncRoute(async(req,res)=>{const warehouseId=req.query.warehouseId?String(req.query.warehouseId):undefined;if(warehouseId)await assertWarehouseAccess(req,warehouseId);let outletName='';if(req.query.outletId){assertOutlet(req,String(req.query.outletId));const outlet=await prisma.outlet.findFirst({where:tenantWhereAnd(req,{id:String(req.query.outletId)}),select:{name:true}});outletName=outlet?.name||'';}const where:any={warehouse:warehouseScope(req),item:tenantWhereAnd(req,{status:'ACTIVE',stockAlertEnabled:true})};if(warehouseId)where.warehouseId=warehouseId;const stocks=await prisma.inventoryStock.findMany({where,include:{item:{include:{category:true,unit:true}}}});const cutoff=new Date(Date.now()-6*60*60*1000);const alerts=[];for(const stock of stocks){const item=stock.item,current=Number(stock.currentQty);if(!inventoryAlertBreached(item,current)){if(item.stockAlertState!=='NORMAL')await prisma.inventoryItem.update({where:{id:item.id},data:{stockAlertState:'NORMAL'}});continue;}if(item.stockAlertState==='ALERTED'&&item.lastStockAlertAt&&item.lastStockAlertAt>cutoff)continue;alerts.push({inventoryItemId:item.id,itemName:item.name,unit:item.unit?.name,alertType:item.stockAlertType,currentStock:current,threshold:inventoryAlertThreshold(item),title:inventoryAlertTitle(item,current),message:inventoryAlertMessage(item,outletName,current)});}res.json(alerts);}));
...
api.put('/coupons/:id',allow('OWNER'),asyncRoute(async(req,res)=>{const d=couponBody.parse(req.body);const {outletIds,productIds,categories,...data}=d;const id=String(req.params.id);const existing=await assertTenantCoupon(req,id);for(const outletId of outletIds)await assertTenantOutlet(req,outletId);for(const productId of productIds)await assertTenantProduct(req,productId);res.json(await prisma.$transaction(async tx=>{await tx.couponOutlet.deleteMany({where:{couponId:id}});await tx.couponProduct.deleteMany({where:{couponId:id}});await tx.couponCategory.deleteMany({where:{couponId:id}});return tx.coupon.update({where:{id},data:{...data,businessId:existing.businessId||req.user!.businessId,outlets:{create:outletIds.map(outletId=>({outletId}))},products:{create:productIds.map(productId=>({productId}))},categories:{create:categories.map(category=>({category}))}},include:{outlets:true,products:true,categories:true}});}));}));
...
function ordersBaseWhere(req:any){const where:any=tenantWhereAnd(req,{outletId:requiredOutletId(req)});const range=queryDateRange(req.query);if(range)where.createdAt=range;return where;}
...
async function reportLines(req:any){
  const outletId=requiredOutletId(req);
...
api.get('/sales',asyncRoute(async(req,res)=>{const where:any=tenantWhereAnd(req,{status:{in:['PAID','VOID']},outletId:requiredOutletId(req)});if(req.query.date)where.createdAt=dayRange(String(req.query.date));if(req.query.payment_method)where.paymentMethod=req.query.payment_method;res.json(await prisma.sale.findMany({where,include:{outlet:true,cashier:{select:{name:true}}},orderBy:{createdAt:'desc'},take:200}));}));
