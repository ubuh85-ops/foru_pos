...
api.post('/auth/login',asyncRoute(async(req,res)=>{
  ...
  res.json({token:jwt.sign(payload,process.env.JWT_SECRET||'dev-secret',{expiresIn:'12h'}),user:{id:user.id,name:user.name,role:user.role,outletIds:payload.outletIds,inventoryPermissions}});
}));
api.get('/auth/me',auth,asyncRoute(async(req,res)=>{const user=await prisma.user.findUnique({where:{id:req.user!.id},select:{id:true,name:true,username:true,role:true,status:true,lastLogin:true,inventoryPermissions:true,outlets:{where:{status:'ACTIVE'},select:{outlet:true}}}});res.json(user?{...user,outletIds:req.user!.outletIds,inventoryPermissions:req.user!.inventoryPermissions}:null);}));

api.use(auth);
const userSelect={id:true,name:true,username:true,role:true,status:true,lastLogin:true,createdAt:true,updatedAt:true,inventoryPermissions:true,outlets:{where:{status:'ACTIVE' as const},include:{outlet:true},orderBy:{outlet:{name:'asc' as const}}}};
...
api.post('/users',allow('OWNER'),asyncRoute(async(req,res)=>{
  ...
  const created=await prisma.$transaction(async tx=>{
    ...
    const user=await tx.user.create({data:{name:d.name,username:d.username,passwordHash,pinHash,role:d.role,status:d.status,inventoryPermissions}});
    await syncUserOutlets(tx,user.id,d.role,d.outletIds);
    await tx.auditLog.create({data:{entityType:'USER',entityId:user.id,action:'USER_CREATED',oldValue:Prisma.JsonNull,newValue:{name:d.name,username:d.username,role:d.role,status:d.status,outletIds:d.outletIds,inventoryPermissions},changedBy:req.user!.id}});
...
api.put('/users/:id',allow('OWNER'),asyncRoute(async(req,res)=>{
  ...
  const updated=await prisma.$transaction(async tx=>{
    ...
    await tx.auditLog.create({data:{entityType:'USER',entityId:id,action:'USER_UPDATED',oldValue:{name:existing.name,username:existing.username,role:existing.role,status:existing.status,inventoryPermissions:existing.inventoryPermissions},newValue:{...data,outletIds:d.outletIds},changedBy:req.user!.id}});
...
function warehouseScope(req:any){return req.user!.role==='OWNER'?{}:{outletId:{in:req.user!.outletIds}};}
async function assertWarehouseAccess(req:any,warehouseId:string,tx:any=prisma){const wh=await tx.inventoryWarehouse.findUnique({where:{id:warehouseId}});if(!wh||wh.status!=='ACTIVE')throw new ApiError(404,'Warehouse tidak ditemukan');if(req.user!.role!=='OWNER'&&(!wh.outletId||!req.user!.outletIds.includes(wh.outletId)))throw new ApiError(403,'Warehouse tidak diizinkan');return wh;}
async function warehouseIdForRequest(req:any,id?:string|null,tx:any=prisma){if(id){await assertWarehouseAccess(req,id,tx);return id;}if(req.user!.role==='OWNER')return warehouseIdOrDefault(null,tx);const wh=await tx.inventoryWarehouse.findFirst({where:{status:'ACTIVE',outletId:{in:req.user!.outletIds}},orderBy:{name:'asc'}});if(!wh)throw new ApiError(403,'Warehouse tidak diizinkan');return wh.id;}
...
api.get('/inventory/items',invView,asyncRoute(async(req,res)=>{const where:any={};if(req.query.q){const q=String(req.query.q);where.OR=[{name:{contains:q,mode:'insensitive'}},{code:{contains:q,mode:'insensitive'}},{sku:{contains:q,mode:'insensitive'}},{barcode:{contains:q,mode:'insensitive'}}];}if(req.query.category_id)where.categoryId=String(req.query.category_id);if(req.query.status)where.status=String(req.query.status);if(req.query.warehouseId){const warehouseId=String(req.query.warehouseId);await assertWarehouseAccess(req,warehouseId);where.stocks={some:{warehouseId}};}res.json(await prisma.inventoryItem.findMany({where,include:inventoryItemInclude,orderBy:{name:'asc'}}));}));
...
api.get('/inventory/transfers',invTransfer,asyncRoute(async(req,res)=>{const where:any={};if(req.query.status)where.status=String(req.query.status);if(req.user!.role!=='OWNER')where.OR=[{fromWarehouse:{outletId:{in:req.user!.outletIds}}},{toWarehouse:{outletId:{in:req.user!.outletIds}}}];if(req.query.warehouseId){await assertWarehouseAccess(req,String(req.query.warehouseId));where.OR=[{fromWarehouseId:String(req.query.warehouseId)},{toWarehouseId:String(req.query.warehouseId)}];}res.json(await prisma.stockTransfer.findMany({where,include:{fromWarehouse:true,toWarehouse:true,creator:{select:{name:true}},completer:{select:{name:true}},items:{include:{item:true}}},orderBy:{createdAt:'desc'},take:200}));}));
...
api.get('/inventory/reports/transfers',invReport,asyncRoute(async(req,res)=>{const where:any={};if(req.user!.role!=='OWNER')where.OR=[{fromWarehouse:{outletId:{in:req.user!.outletIds}}},{toWarehouse:{outletId:{in:req.user!.outletIds}}}];if(req.query.from&&req.query.to)where.createdAt=inventoryDateRange(req.query);res.json(await prisma.stockTransfer.findMany({where,include:{fromWarehouse:true,toWarehouse:true,items:{include:{item:true}}},orderBy:{createdAt:'desc'},take:300}));}));
