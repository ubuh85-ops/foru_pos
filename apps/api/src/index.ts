...
api.post('/auth/login',asyncRoute(async(req,res)=>{
  ...
  const inventoryPermissions=user.inventoryPermissions.length?user.inventoryPermissions:defaultInventoryPermissions(user.role);
  const payload={id:user.id,role:user.role,outletIds,assignedWarehouseId:user.assignedWarehouseId};
  await prisma.user.update({where:{id:user.id},data:{lastLogin:new Date()}});
  res.json({token:jwt.sign(payload,process.env.JWT_SECRET||'dev-secret',{expiresIn:'12h'}),user:{id:user.id,name:user.name,role:user.role,outletIds:payload.outletIds,inventoryPermissions,assignedWarehouseId:user.assignedWarehouseId}});
}));
api.get('/auth/me',auth,asyncRoute(async(req,res)=>{const user=await prisma.user.findUnique({where:{id:req.user!.id},select:{id:true,name:true,username:true,role:true,status:true,lastLogin:true,inventoryPermissions:true,assignedWarehouseId:true,assignedWarehouse:true,outlets:{where:{status:'ACTIVE'},select:{outlet:true}}}});res.json(user?{...user,outletIds:req.user!.outletIds,inventoryPermissions:req.user!.inventoryPermissions,assignedWarehouseId:req.user!.assignedWarehouseId}:null);}));
...
function warehouseScope(req:any){
  ...
  return {outletId:{in:req.user!.outletIds}};
}
...
async function warehouseIdForRequest(req:any,id?:string|null,tx:any=prisma){if(id){await assertWarehouseAccess(req,id,tx);return id;}if(req.user!.role==='OWNER')return warehouseIdOrDefault(null,tx);if(req.user!.assignedWarehouseId){await assertWarehouseAccess(req,req.user!.assignedWarehouseId,tx);return req.user!.assignedWarehouseId;}const wh=await tx.inventoryWarehouse.findFirst({where:{status:'ACTIVE',outletId:{in:req.user!.outletIds}},orderBy:{name:'asc'}});if(!wh)throw new ApiError(403,'Warehouse tidak diizinkan');return wh.id;}
async function auditInventory(tx:any,req:any,action:string,warehouseId:string,newValue:any){const wh=await tx.inventoryWarehouse.findUnique({where:{id:warehouseId},include:{outlet:true}});await tx.auditLog.create({data:{entityType:'INVENTORY',entityId:warehouseId,action,oldValue:Prisma.JsonNull,newValue:{...newValue,user:req.user!.id,role:req.user!.role,warehouse:wh?.name,outletId:wh?.outletId,outlet:wh?.outlet?.name},changedBy:req.user!.id}});}
...
api.post('/inventory/categories',invItemManagement,asyncRoute(async(req,res)=>{const d=inventoryLookupBody.parse(req.body);if(await prisma.inventoryCategory.findUnique({where:{name:d.name}}))throw new ApiError(409,'Kategori inventory sudah ada.');res.status(201).json(await prisma.inventoryCategory.create({data:d}));}));
...
api.post('/inventory/units',invItemManagement,asyncRoute(async(req,res)=>{const d=inventoryLookupBody.parse(req.body);if(await prisma.inventoryUnit.findUnique({where:{name:d.name}}))throw new ApiError(409,'Satuan inventory sudah ada.');res.status(201).json(await prisma.inventoryUnit.create({data:d}));}));
