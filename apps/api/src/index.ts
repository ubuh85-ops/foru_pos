...
function warehouseScope(req:any){return req.user!.role==='OWNER'?{}:(req.user!.assignedWarehouseId?{id:req.user!.assignedWarehouseId}:{OR:[{outletId:{in:req.user!.outletIds}},{outletId:null}]});}
...
api.get('/inventory/items',invView,asyncRoute(async(req,res)=>{const where:any={};let warehouseId:string|undefined;if(req.query.q){const q=String(req.query.q);where.OR=[{name:{contains:q,mode:'insensitive'}},{code:{contains:q,mode:'insensitive'}},{sku:{contains:q,mode:'insensitive'}},{barcode:{contains:q,mode:'insensitive'}}];}if(req.query.category_id)where.categoryId=String(req.query.category_id);if(req.query.status)where.status=String(req.query.status);if(req.query.warehouseId){warehouseId=String(req.query.warehouseId);await assertWarehouseAccess(req,warehouseId);}res.json(await prisma.inventoryItem.findMany({where,include:inventoryItemIncludeFor(req,warehouseId),orderBy:{name:'asc'}}));}));
