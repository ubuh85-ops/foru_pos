...
import { allow, ApiError, assertOutlet, asyncRoute, auth, dayRange, defaultBusinessForUser, defaultInventoryPermissions, hasPermission, money, prisma, requirePermission } from './lib.js';
...
async function assertNotLastOwner(userId:string,next?:{role?:string,status?:string}){
  const user=await prisma.user.findUnique({where:{id:userId},include:{outlets:true}});
...
async function validateUserWarehouse(role:string,outletIds:string[],assignedWarehouseId?:string|null){
...
api.post('/users',allow('OWNER'),asyncRoute(async(req,res)=>{
  ...
  const assignedWarehouseId=await validateUserWarehouse(d.role,d.outletIds,d.assignedWarehouseId);
...
async function assertWarehouseAccess(req:any,warehouseId:string,tx:any=prisma){const wh=await tx.inventoryWarehouse.findUnique({where:{id:warehouseId}});if(!wh||wh.status!=='ACTIVE')throw new ApiError(404,'Warehouse tidak ditemukan');if(req.user!.role!=='OWNER'){if(req.user!.assignedWarehouseId&&warehouseId!==req.user!.assignedWarehouseId)throw new ApiError(403,'Warehouse tidak diizinkan');if(!req.user!.assignedWarehouseId&&wh.outletId&&!req.user!.outletIds.includes(wh.outletId))throw new ApiError(403,'Warehouse tidak diizinkan');}return wh;}
...
api.get('/warehouses',invView,asyncRoute(async(req,res)=>{const where:any={...warehouseScope(req)};if(req.query.status)where.status=String(req.query.status);if(req.query.q){const q=String(req.query.q);where.OR=[{name:{contains:q,mode:'insensitive'}},{code:{contains:q,mode:'insensitive'}}];}res.json(await prisma.inventoryWarehouse.findMany({where,include:{outlet:true},orderBy:{name:'asc'}}));}));
...
api.get('/inventory/categories',invView,asyncRoute(async(_req,res)=>res.json(await prisma.inventoryCategory.findMany({orderBy:[{sortOrder:'asc'},{name:'asc'}]}))));
...
api.get('/inventory/units',invView,asyncRoute(async(_req,res)=>res.json(await prisma.inventoryUnit.findMany({orderBy:[{sortOrder:'asc'},{name:'asc'}]}))));
...
api.get('/inventory/items',invView,asyncRoute(async(req,res)=>{const where:any={};let warehouseId:string|undefined;if(req.query.q){const q=String(req.query.q);where.OR=[{name:{contains:q,mode:'insensitive'}},{code:{contains:q,mode:'insensitive'}},{sku:{contains:q,mode:'insensitive'}},{barcode:{contains:q,mode:'insensitive'}}];}if(req.query.category_id)where.categoryId=String(req.query.category_id);if(req.query.status)where.status=String(req.query.status);if(req.query.warehouseId){warehouseId=String(req.query.warehouseId);await assertWarehouseAccess(req,warehouseId);}res.json(await prisma.inventoryItem.findMany({where:{...where,...(warehouseId?{stocks:{some:{warehouseId}}}:{})},include:inventoryItemIncludeFor(req,warehouseId),orderBy:{name:'asc'}}));}));
api.get('/inventory/search',invView,asyncRoute(async(req,res)=>{const q=String(req.query.q||req.query.sku||req.query.barcode||'').trim();if(!q)throw new ApiError(400,'Query wajib diisi.');const warehouseId=req.query.warehouseId?String(req.query.warehouseId):undefined;if(warehouseId)await assertWarehouseAccess(req,warehouseId);res.json(await prisma.inventoryItem.findMany({where:{OR:[{sku:{equals:q,mode:'insensitive'}},{barcode:{equals:q,mode:'insensitive'}},{code:{equals:q,mode:'insensitive'}},{sku:{contains:q,mode:'insensitive'}},{barcode:{contains:q,mode:'insensitive'}},{code:{contains:q,mode:'insensitive'}},{name:{contains:q,mode:'insensitive'}}]},include:inventoryItemIncludeFor(req,warehouseId),orderBy:[{sku:'asc'},{name:'asc'}],take:20}));}));
...
api.post('/inventory/items/:id/assign-warehouses',invItemManagement,asyncRoute(async(req,res)=>{
  ...
  const item=await prisma.inventoryItem.findUnique({where:{id},include:{stocks:true}});
...
api.get('/inventory/transfers',invTransfer,asyncRoute(async(req,res)=>{const where:any={};if(req.query.status)where.status=String(req.query.status);if(req.user!.role!=='OWNER')where.OR=req.user!.assignedWarehouseId?[{fromWarehouseId:req.user!.assignedWarehouseId},{toWarehouseId:req.user!.assignedWarehouseId}]:[{fromWarehouse:{outletId:{in:req.user!.outletIds}}},{toWarehouse:{outletId:{in:req.user!.outletIds}}}];if(req.query.warehouseId){await assertWarehouseAccess(req,String(req.query.warehouseId));where.OR=[{fromWarehouseId:String(req.query.warehouseId)},{toWarehouseId:String(req.query.warehouseId)}];}res.json(await prisma.stockTransfer.findMany({where,include:{fromWarehouse:true,toWarehouse:true,creator:{select:{name:true}},completer:{select:{name:true}},items:{include:{item:true}}},orderBy:{createdAt:'desc'},take:200}));}));
...
api.get('/inventory/reports/transfers',invReport,asyncRoute(async(req,res)=>{const where:any={};if(req.user!.role!=='OWNER')where.OR=req.user!.assignedWarehouseId?[{fromWarehouseId:req.user!.assignedWarehouseId},{toWarehouseId:req.user!.assignedWarehouseId}]:[{fromWarehouse:{outletId:{in:req.user!.outletIds}}},{toWarehouse:{outletId:{in:req.user!.outletIds}}}];if(req.query.from&&req.query.to)where.createdAt=inventoryDateRange(req.query);res.json(await prisma.stockTransfer.findMany({where,include:{fromWarehouse:true,toWarehouse:true,items:{include:{item:true}}},orderBy:{createdAt:'desc'},take:300}));}));
...
async function buildOrderTotals(d:z.infer<typeof saleInput>){
  const lines=await priceCart(d.items,d.outletId,d.orderType);
...
async function reportLines(req:any){
  ...
  const saleWhere:any={outletId,status:'PAID'};
