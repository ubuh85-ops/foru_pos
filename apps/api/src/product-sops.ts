...
function present(product: Prisma.ProductGetPayload<{ include: typeof productInclude }>) {
  ...
  return {
    ...
    recipe: product.recipes.map(row => ({
      id: row.id,
      sourceType: row.componentProductId ? 'PRODUCT' : 'INVENTORY',
      ingredientId: row.componentProduct?.id || row.item?.id,
      ingredientCode: row.componentProduct?.sku || row.item?.code,
      ingredientName: row.componentProduct?.name || row.item?.name || '-',
      qty: Number(row.usageQty),
      unit: row.componentProductId ? row.componentUnit || 'unit' : row.usageUnit?.name || '-',
