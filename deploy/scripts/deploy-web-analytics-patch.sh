...
PATCH_FILE="${1:-$PATCH_DIR/web-analytics.patch}"
EXPECTED_BASE=455f3a1
cd "$APP_DIR"
test -f "$PATCH_FILE"
...
BACKUP="$APP_DIR/deploy/backups/web-analytics-$STAMP"
mkdir -p "$BACKUP"
chmod 700 "$BACKUP"
...
for SERVICE in api web; do
  ...
  docker tag "$IMAGE" "foru-$SERVICE:before-web-analytics-$STAMP"
